...
export const exportQuoteToPDF = (
  ...
  doc.text("Qtà", pageWidth - margin - 60, yPosition);
  doc.text(
    "Prezzo Unit.",
    pageWidth - margin - 45,
    yPosition
  );
  doc.text("Totale", pageWidth - margin - 20, yPosition, {
    align: "right",
  });
  yPosition += 10;
  ...
  doc.text(
    `Preventivo generato il ${new Date().toLocaleString(
      "it-IT"
    )}`,
    pageWidth / 2,
    pageHeight - 10,
  ...
  // Salva il PDF
  const fileName = `Preventivo_${quote.patientName.replace(
    /\s+/g,
    "_"
