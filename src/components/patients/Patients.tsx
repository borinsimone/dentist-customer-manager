...
import { patientsService, generateId } from '../../services/storage';
import type { Patient, ReminderChannel } from '../../types';
...
const getChannelLabel = (channel?: ReminderChannel) => {
  ...
  return channel ? labels[channel] : 'Email';
};
...
export const Patients = () => {
  ...
  return (
    <Layout
      title='Pazienti'
      actions={<Button onClick={handleAddPatient}>➕ Aggiungi Paziente</Button>}
    >
              ...
              {filteredPatients.map((patient, index) => (
                ...
                >
                  ...
                  <div className={styles['patient-info']}>
                    <div className={styles['patient-name']}>{patient.name}</div>
                    <div className={styles['patient-contact']}>
                    ...
                    {patient.clinicalNotes && (
                      <div className={styles['patient-notes']}>
                        {patient.clinicalNotes.substring(0, 100)}
                        {patient.clinicalNotes.length > 100 ? '...' : ''}
                      </div>
                    )}
                    <div className={styles['reminder-channel']}>
                      🔔 Preferenza: {getChannelLabel(patient.reminderChannel)}
                    </div>
                  </div>
...
const PatientModal = ({ patient, onClose, onSave }: PatientModalProps) => {
  const [formData, setFormData] = useState({
    ...
    reminderChannel: patient?.reminderChannel || 'email',
  });
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          <div className={styles['modal-body']}>
            ...
            <div className={styles['form-group']}>
              <label htmlFor='clinicalNotes'>Note cliniche</label>
              <textarea
                id='clinicalNotes'
                name='clinicalNotes'
