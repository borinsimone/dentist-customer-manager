...
const STORAGE_KEYS = {
  ...
  REMINDER_SETTINGS: 'dentist_reminder_settings',
  REMINDER_LOGS: 'dentist_reminder_logs',
};
...
export const patientsService = new LocalStorageService<Patient>(
  STORAGE_KEYS.PATIENTS,
);
export const documentsService = new LocalStorageService<Document>(
  STORAGE_KEYS.DOCUMENTS,
);
export const treatmentsService = new LocalStorageService<Treatment>(
  STORAGE_KEYS.TREATMENTS,
);
export const appointmentsService = new LocalStorageService<Appointment>(
  STORAGE_KEYS.APPOINTMENTS,
);
export const quotesService = new LocalStorageService<Quote>(
  STORAGE_KEYS.QUOTES,
);
export const paymentsService = new LocalStorageService<Payment>(
  STORAGE_KEYS.PAYMENTS,
);
export const treatmentPricesService = new LocalStorageService<TreatmentPrice>(
  STORAGE_KEYS.TREATMENT_PRICES,
);
export const reminderLogsService = new LocalStorageService<ReminderLog>(
...
export const reminderSettingsService = {
  get(): ReminderSettings {
    ...
    try {
      ...
      return {
        autoSendEnabled:
          parsed.autoSendEnabled ?? DEFAULT_REMINDER_SETTINGS.autoSendEnabled,
        hoursBefore:
          parsed.hoursBefore ?? DEFAULT_REMINDER_SETTINGS.hoursBefore,
      };
    } catch (error) {
      console.error('Errore reminder settings:', error);
      localStorage.setItem(
        STORAGE_KEYS.REMINDER_SETTINGS,
        JSON.stringify(DEFAULT_REMINDER_SETTINGS),
      );
  ...
  update(updates: Partial<ReminderSettings>) {
    const current = reminderSettingsService.get();
    const next = { ...current, ...updates };
    localStorage.setItem(STORAGE_KEYS.REMINDER_SETTINGS, JSON.stringify(next));
    return next;
  },
};
...
export const backupData = (): string => {
  const backup = {
    ...
    reminderSettings: reminderSettingsService.get(),
    reminderLogs: reminderLogsService.getAll(),
...
export const restoreData = (backupJson: string): boolean => {
  try {
    ...
    localStorage.setItem(
      STORAGE_KEYS.REMINDER_SETTINGS,
      JSON.stringify(backup.reminderSettings || DEFAULT_REMINDER_SETTINGS),
    );
    localStorage.setItem(
      STORAGE_KEYS.REMINDER_LOGS,
      JSON.stringify(backup.reminderLogs || []),
    );
...
export const initializeDemoData = () => {
  if (patientsService.getAll().length === 0) {
    ...
    defaultPrices.forEach((price) => treatmentPricesService.create(price));
  }
};
